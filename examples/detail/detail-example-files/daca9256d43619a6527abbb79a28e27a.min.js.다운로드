$(document).ready(function(){$.TGD=$.TGD||{};$.TGD.latest=function(){var cookie_id='rclist';var list=[];var max_length=10;return{init:function(noAdd){$('#latestDropdownMenu').empty()
...
e.stopPropagation()
$.TGD.latest.remove($(this).parents('a').attr('href').slice(1))});$('#favoriteDropdown').click(function(e){$('#favoriteDropdownMenu').empty()
$.getJSON('/board/favorite_load',function(r){for(var i=0;i<r.data.length;++i)
$('#favoriteDropdownMenu').append('<li><a data-id="'+r.data[i].favorite_id+'" href="/'+r.data[i].alias+'"><span class="name">'+r.data[i].name+'</span> <span class="delrow"><i class="fa fa-remove"></i></span></a></li>')})})
$(document).on('click','#favoriteDropdownMenu > li > a > span.delrow',function(e){e.preventDefault()
e.stopPropagation()
$.post('/member/config/fav',{remove:[$(this).parents('a').attr('data-id')]},function(r){$('#favoriteDropdown').trigger('click');setTimeout(function(){$('#favoriteDropdown').trigger('click')},100)})});if($('#notification-badge').length>0){$.getJSON('/notify/load',function(r){if(r.count>0){if(r.count>99)
...
var $window=$(window);$window.scroll(function(){$('#movetotop')[$window.scrollTop()>100?'fadeIn':'fadeOut']();$('#weare-out-of-food,#plz-dont-block-me').css('top',Math.max(0,106-$window.scrollTop()))})
$('#movetotop').click(function(e){e.preventDefault()
$(window).scrollTop(0)})
var onAdResize=function(){$('#weare-out-of-food,#plz-dont-block-me').css('width',Math.min(300,($window.width()-1100)/2))}
...
$.post('/board/favorite',{board_id:$(this).attr('data-id')},function(r){if(r.status!==200){alertify.reset();alertify.error(r.message)}else{$('#addFavoriteBtn > i').removeClass('fa-star-o fa-star').addClass(r.has?'fa-star-o':'fa-star');alertify.reset();alertify.success(!r.has?'즐겨찾기에 추가했습니다.':'즐겨찾기에서 제거했습니다.')}},'json')})
...
if(lists.length<=0){alertify.reset();alertify.error('삭제할 글을 선택해주세요.')}else{alertify.reset();alertify.confirm('정말 삭제하시겠습니까? 삭제 후 복구가 불가 합니다.',function(r){$.post('/board/delete_all/'+alias,{lists:lists},function(r){if(r.status!==200){alertify.reset();alertify.alert('에러 발생 ('+r.status+')')}else{alertify.reset();alertify.alert('총 '+r.deleted+' 건의 글을 삭제했습니다.',function(rrr){location.href='/'+alias})}})})}})});$(document).ready(function(){$.TGD=$.TGD||{};$.TGD.commentList=function(){var $area=$('#reply-real-area');var inited=!1;var cpage=undefined;var lastComment=null;var onPrevPageButtonClicked=function(e){if(cpage<=1)
return;if(e!==undefined)
e.preventDefault()
load(cpage-1,!0)},onLoadNewCommentsButtonClicked=function(e){$.TGD.newComment.count=0;$('#loadNewCommentsBtn').addClass('hide');lastComment=$('div.reply').last().attr('id');load(parseInt($('#article-content').attr('data-last-cpage'),10),!1,function(){if(lastComment!==undefined){location.href='#'+lastComment}else{location.href='#'+$('div.reply').last().attr('id')}},!0);if(e!==undefined)
e.preventDefault()},check=function(){return},load=function(_cpage,isAppend,cb,forceCache){var loadPrefix='';if(forceCache===!0){loadPrefix='?cache=true'}
if(!inited){$('#viewReplyPrevPageBtn').click(onPrevPageButtonClicked);$('#loadNewCommentsBtn').click(onLoadNewCommentsButtonClicked);setInterval(check,30*1000);inited=!0;loadPrefix='?cache=true'}
lastTotalRows=null;if(isAppend!==!0)
$area.empty()
cpage=_cpage;$('#viewReplyPrevPageBtn')[cpage>1?'removeClass':'addClass']('hide')
$.getJSON('/board/comment_load/'+$('#article-content').attr('data-id')+'/'+cpage+loadPrefix,function(res){render(res,cb)})},renderIt=function(data,returnElement){var $profileImage=$('<img />').addClass('reply-writer-image img-circle').attr('data-src',data.logo).attr('src','//placehold.it/1x1').attr('width',48).attr('height',48);var $code=$('<small class="code" />').text(data.code);var $writer=$('<div class="reply-writer" />').addClass(data.logged?'logged':'').html(data.icon+' '+data.nickname);var $likeBtn=$('<a href="/board/comment_vote" data-id="'+data.id+'" data-type="up" class="comment-vote-btn" />').html(parseInt(data.up,10)?'<span class="like"><i class="fa fa-thumbs-up"></i> '+data.up+'</span>':'좋아요');var $rereplyBtn=$('<a href="#" class="re-reply-btn" />').text('대댓글')
var $created_at=$('<span class="time" />').attr('title',data.created_at).text(moment(data.created_at).from(now));var $replyMenu=$('<div class="reply-menu" />');if(data.owned===!0){var $editBtn=$('<a href="/board/comment_edit/'+data.id+'" />').text('수정');var $deleteBtn=$('<a href="/board/comment_delete/'+data.id+'" />').text('삭제');$replyMenu.append($editBtn).append($('<span class="middot" />').html('&middot;')).append($deleteBtn).append($('<span class="middot" />').html('&middot;'))}
$replyMenu.append($likeBtn).append($('<span class="middot" />').html('&middot;')).append(data.best?'':$rereplyBtn).append(data.best?'':$('<span class="middot" />').html('&middot;')).append($created_at);var $replyHeader=$('<div class="reply-header" />').append($profileImage).append($writer).append($code).append($replyMenu).append('<div class="clearfix" />');var $replyContent=$('<div class="reply-content" />').html(data.content.autoLink())
if(data.best){$writer.prepend('<span class="label label-danger">BEST</span>&nbsp;')}
$replyContent.kappa({})
$('img',$replyContent).each(function(i,img){var $img=$(img);if($img.attr('data-original-gif')===undefined&&/upload\.tgd\.kr\/.*\.gif$/.test($img.attr('data-src'))){$img.removeClass('fr-dii fr-dib fr-fil fr-fic').attr('data-original-gif',$img.attr('data-src').replace(/(upload\.tgd\.kr\/[0-9]+)\//,'$1/o_')).wrap('<a href="#" class="gif-thumbnail '+$img.attr('class')+'"></a>')}})
var $reply=$('<div class="reply" />').attr('id',(data.best?'best-':'')+'comment-'+data.id).attr('data-id',data.id).addClass('d-'+Math.min(4,data.depth));if(data.secret===!0){$reply.addClass('secret')}
if(data.depth>0){$reply.append('<a href="#comment-'+data.parent+'" class="rereply-btn"><i class="fa fa-share"></i></a>')}
$reply.append($replyHeader).append($replyContent);if(returnElement===!0){return $reply}else{$area.prepend($reply)}},render=function(res,cb){$('#totalCommentCount').text(res.total_rows);var now=moment().tz('Asia/Seoul');var data=res.data;if(data.length<=0){if(typeof cb==='function')
cb();return}
for(var i=data.length-1;i>=0;--i){renderIt(data[i])}
if(res.best){$('#reply-best-area').empty();for(var i=0;i<res.best.length;i+=1){$('#reply-best-area').append($.TGD.commentList.renderIt(res.best[i],!0))}}
if(myLazyLoad)myLazyLoad.update();if(typeof cb==='function')
cb()},goPrevPage=function(cb){load(cpage-1,!0,cb)},findComment=function(selector){var $hash=$(selector);if($hash.length>0){$('div.reply.active').removeClass('active')
$hash.addClass('active')
if(!$hash.data('visited')){$(window).scrollTop($hash.offset().top)
$hash.data('visited',!0)}}else{if(!$('#viewReplyPrevPageBtn').hasClass('hide')){goPrevPage(function(){findComment(selector)})}}};return{onPrevPageButtonClicked:onPrevPageButtonClicked,load:load,render:render,renderIt:renderIt,findComment:findComment,goPrevPage:goPrevPage}}();$.TGD.commentForm=function(){var onCallback=function(data){$('#writeCommentBtn').attr('disabled',!1);if(data.status===200){var isReReply=$('#commentParentId').val()!=='0';onRereplyCancelBtnClicked(null);if(isReReply===!0){$.TGD.commentList.load(data.total_page,!1,function(){location.hash='#comment-'+data.id})}else{$('#reply-real-area').append($.TGD.commentList.renderIt(data.data,!0));if(myLazyLoad)myLazyLoad.update();location.hash='#comment-'+data.id}
$('#article-content').attr('data-last-cpage',data.total_page)}else if(data.status===403){alertify.reset();alertify.confirm(data.message,function(){location.href=data.link})}else if(data.status===500){alertify.reset();alertify.error(data.message)}},onRereplyCancelBtnClicked=function(e){if(e!==null)
e.preventDefault()
$('#commentContent').froalaEditor('html.set','');$('#comment-write-form').appendTo('#comment-write-form-area')
$('#commentParentId').val(0)
$('#secret').prop('checked',!1);$('#btnRereplyCancelBtn').addClass('hide')
if(e!==null)
$('#commentContent').focus()};return{onRereplyCancelBtnClicked:onRereplyCancelBtnClicked,onCallback:onCallback,init:function(){if($('#userDropdown > img.profile-img').length<=0){$('#commentWriteInfoForm').removeClass('hide')
if(Cookies.get('w_nick'))$('#commentNickname').val(Cookies.get('w_nick'));if(Cookies.get('w_pass'))$('#commentPassword').val(Cookies.get('w_pass'))}
$('#comment-write-form').submit(this.onSubmit);$('#commentContent').froalaEditor({key:'7A3B3D3I3cA5A4B3F2E4C2B2E3D1B6vE-13kiH5db==',placeholderText:'내용을 작성해주세요.',end_with_newline:!0,indent_inner_html:!0,extra_liners:['p','h1','h2','h3','h4','h5','h6','blockquote','pre','ul','ol','table','dl'],brace_style:'expand',indent_char:'\t',indent_size:4,wrap_line_length:0,tooltips:!0,paragraphFormatSelection:!0,toolbarStickyOffset:0,quickInsertTags:['p','div','blockquote'],quickInsertButtons:['ul','ol','hr'],pasteAllowLocalImages:!1,imagePasteProcess:!1,imageEditButtons:['imageSize'],imageDefaultAlign:'left',videoUpload:!1,height:150,language:'ko',toolbarButtons:['html','|','emoticons','bold','italic','underline','strikeThrough','color','|','insertLink','insertImage','insertVideo','insertTable','|','formatOL','formatUL'],toolbarButtonsXS:['html','|','emoticons','bold','italic','underline','strikeThrough','color','|','insertLink','insertImage','insertVideo','insertTable','|','formatOL','formatUL'],fileUploadURL:'/pages/file_upload',imageUploadURL:'/pages/file_upload',imageDefaultWidth:0})
$('#btnRereplyCancelBtn').click(this.onRereplyCancelBtnClicked);$(document).on('click','a.re-reply-btn',this.onRereplyBtnClicked)},onRereplyBtnClicked:function(e){if(e!==null)
e.preventDefault()
var $parent=$(this).parents('div.reply')
$('#secret').prop('checked',$parent.hasClass('secret'));$('#commentContent').froalaEditor('html.set','');$('#comment-write-form').appendTo($('div.reply-content',$parent))
$('#commentParentId').val($parent.attr('data-id'))
$('#btnRereplyCancelBtn').removeClass('hide')
$('#commentContent').focus()},onSubmit:function(e){$.TGD.newComment.time=(new Date().getTime()/1000+3);var $form=$('#comment-write-form');e.preventDefault()
if($('#commentWriteInfoForm').hasClass('hide')===!1){Cookies.set('w_nick',$('#commentNickname').val(),{expires:60});Cookies.set('w_pass',$('#commentPassword').val(),{expires:60})}
$('#writeCommentBtn').attr('disabled',!0);$.post($form.attr('action'),$form.serialize(),onCallback,'json')}}}();window.onhashchange=function(){if(location.hash==="")
return;$.TGD.commentList.findComment(location.hash)};$.TGD.commentList.load(parseInt($('#article-content').attr('data-last-cpage'),10),!1,function(){window.onhashchange()});$.TGD.commentForm.init();$('#lastc').click(function(e){e.preventDefault();if($('div.reply').last().attr('id')!==undefined){location.href='#'+$('div.reply').last().attr('id')}});$('button.vote-btn').click(function(e){e.preventDefault()
if($(this).data('pressed'))return;$(this).data('pressed',!0);var id=$(this).attr('data-id');var type=$(this).attr('data-type');var $this=$(this);var done=function(){if(type==='up'){$('#up').text(''+(parseInt($('#up').text(),10)+1))}else{$('#down').text(''+(parseInt($('#down').text(),10)-1))}
$.post('/board/vote',{id:id,type:type},function(r){if(r.status===500){alertify.reset();alertify.error(r.message);if(type==='up'){$('#up').text(''+(parseInt($('#up').text(),10)-1))}else{$('#down').text(''+(parseInt($('#down').text(),10)+1))}}
$this.data('pressed',!1)},'json')};if($(this).hasClass('btn-outline-primary')){alertify.reset();alertify.confirm('정말 비추하시겠습니까?',function(a){done()})}else{done()}})
$('a.report-btn').click(function(e){e.preventDefault()
$('#reportModal').modal('show')})
$('#doReportBtn').click(function(e){var href=$('a.report-btn').attr('href');var reason=$('input[name=reason]:checked').val();if(!reason){alertify.error('신고 사유를 선택해주세요.')}
alertify.reset();alertify.confirm('정말 이 글을 신고하시겠습니까? 허위 신고 시 트게더 이용이 제한됩니다').then(function(resolvedValue){if(resolvedValue.buttonClicked==='ok'){$('#reportModal').modal('hide')
$.post(href,{reason:reason},function(r){if(r.status===200){alertify.success('신고가 접수되었습니다.')}else if(r.status===500){alertify.error(r.message)}})}})})
$('a.scrap-btn').click(function(e){e.preventDefault()
var href=$(this).attr('href');alertify.reset();alertify.confirm('이 글을 스크랩하시겠습니까? (스크랩을 해제하려면 스크랩 버튼을 다시 누르세요.)').then(function(resolvedValue){if(resolvedValue.buttonClicked==='ok'){location.href=href}})})
$(document).on('click','a.comment-vote-btn',function(e){e.preventDefault()
$.post($(this).attr('href'),{id:$(this).attr('data-id'),type:$(this).attr('data-type')},function(r){if(r.status===200){alertify.reset();alertify.success(r.message)}else if(r.status===500){alertify.reset();alertify.error(r.message)}},'json')})
$(document).on('click','a.comment-report-btn',function(e){e.preventDefault()
var href=$(this).attr('href');alertify.reset();alertify.confirm('정말 이 댓글을 신고하시겠습니까?').then(function(resolvedValue){if(resolvedValue.buttonClicked==='ok'){$.getJSON(href,function(r){if(r.status===200){alertify.success(r.message)}else if(r.status===500){alertify.error(r.message)}})}})})
$('#article-content').html($('#article-content').html().autoLink({target:"_blank",rel:"nofollow"})).kappa();var now=moment().tz('Asia/Seoul');$('span.ago').each(function(){$(this).text(moment($(this).attr('title')).from(now))})
var myLazyLoad=new LazyLoad({container:document.getElementById('main-content'),threshold:300,});if($('#hgc-container').length>0){$('#article-content img').each(function(){var realsrc=$(this).attr('data-src');if(realsrc===undefined||$(this).attr('has-lazy')===undefined){return}
$(this).attr('src',realsrc)})}
$('#article-content img').each(function(i,img){var $img=$(img);if($img.attr('data-original-gif')===undefined&&/upload\.tgd\.kr\/.*\.gif$/.test($img.attr('data-src'))){$img.removeClass('fr-dii fr-dib fr-fil fr-fic').attr('data-original-gif',$img.attr('data-src').replace(/(upload\.tgd\.kr\/[0-9]+)\//,'$1/o_')).wrap('<a href="#" class="gif-thumbnail '+$img.attr('class')+'"></a>')}})
$(document).on('click','a.gif-thumbnail',function(e){e.preventDefault()
if($(this).hasClass('clicked'))return;$(this).addClass('clicked');var $img=$('img[data-original-gif]',this);$img.attr('src',$img.attr('data-original-gif'))})
$(document).on("click","#reply-real-area div.reply-writer.logged",function(e){var bodyRect=$('#main-content')[0].getBoundingClientRect(),elemRect=$(e.target)[0].getBoundingClientRect();var _left=elemRect.left-bodyRect.left+45;var _top=elemRect.top-bodyRect.top+20;var $menu=$("#writerContextMenu").data("invokedOn",$(e.target)).show().css({position:"absolute",left:_left,top:_top}).off('click').on('click','a',function(e){$menu.hide();e.preventDefault();var $invokedOn=$menu.data("invokedOn");var $selectedMenu=$(e.target);var _sel=$selectedMenu.text();var _who=$invokedOn.text();if(_sel==='작성 글 보기'){$('input[name=search_term]').val(_who.trim());$('input[name=search_type]').val('nickname');$('#boardSearchForm').submit()}else if(_sel==='차단하기'){var ignoreNick=_who.trim();alertify.reset();alertify.confirm('정말 차단하시겠습니까?',function(r){$.post('/board/ignore/',{nick:ignoreNick},function(r){alertify.reset();if(r.status!==200){alertify.alert(r.message)}else{alertify.alert('차단이 완료되었습니다.',function(){location.reload()})}})})}});return!1});try{var board_id=$('#board-info').attr('data-alias');var prefixBadge=localStorage.getItem('badge:'+board_id);if(prefixBadge){$('#custom_badge').val(prefixBadge)}}catch(e){}
$(document).on('click','#showResultVoteBtn',function(e){e.preventDefault()
$('#choiceList').addClass('hide')
$('#choiceResult').removeClass('hide')
$('#doVoteBtn').addClass('hide')
$(this).prop('disabled',!0);setTimeout(function(){$('#showResultVoteBtn').prop('disabled',!1)},3000);$.get('/board/poll_result/'+$('#article-content').attr('data-id'),{key:$('#choiceResult').attr('data-key')},function(r){$('#choiceResult').empty();for(var i=0;i<r.length;i+=1){$('#choiceResult').append('<li class="list-group-item"><span class="badge">'+r[i].count+'</span>'+r[i].text+'</li>')}},'json')})
$(document).on('submit','#article-poll-form',function(e){e.preventDefault()
if($('input.poll-choices:checked').length<1){alertify.confirm('투표하실 항목을 선택해주세요.');return}
$('#doVoteBtn').addClass('hide')
$('#showResultVoteBtn').prop('disabled',!0)
$.post($(this).attr('action'),$(this).serialize(),function(r){$('#showResultVoteBtn').prop('disabled',!1)
if(r.status===403){alertify.confirm('투표를 하려면 로그인 해 주세요.')}else if(r.status===404){alertify.confirm('투표를 찾을 수 없습니다.')}else if(r.status===222){alertify.confirm('투표 가능 갯수를 초과했습니다.');$('#doVoteBtn').removeClass('hide')}else if(r.status===401){alertify.confirm('이미 투표를 하셨습니다.');$('#showResultVoteBtn').trigger('click')}else if(r.status===200){alertify.success('투표 완료!');$('#showResultVoteBtn').trigger('click')}},'json')})
$(document).on('click','#doVoteBtn',function(e){e.preventDefault()
$('#article-poll-form').submit()})})